...
def get_tpcds_query_nodes(job_num = 1):
    ...
    if job_num == 1:
        ...
        wr = ExecutionNode("wr",
            lambda ws: tablereader.read_table("web_returns",
                                              ["wr_item_sk",
                                               ...
                                               "wr_pricing_net_loss"])
                .join(ws.select([pl.col("wsr_web_site_sk"),
                                 pl.col("ws_item_sk"),
        ...
        ssr = ExecutionNode("ssr",
            lambda salesreturns1: tablereader.read_table("date",
                ...
                .join(salesreturns1,
                      left_on = "d_date_sk", right_on = "date_sk")
        ...
        wsr = ExecutionNode("wsr",
            lambda salesreturns3: tablereader.read_table("date",
                ...
                .join(salesreturns3, left_on = "d_date_sk",
                      right_on = "date_sk")
                .join(tablereader.read_table("web_site", ["web_site_sk",
                                                          "web_site_id"]),
        ...
        ws_groupby = ExecutionNode("ws_groupby",
            lambda ws: ws
                .select([pl.col("date_sk"),
                         pl.col("ws_web_page_sk"),
                         pl.col("sales_price"),
                         pl.col("profit")])
                ...
                .join(tablereader.read_table("web_page", ["wp_page_sk"]),
        ...
        ssr2 = ExecutionNode("ssr2",
            lambda ss, sr: ss
                .select([pl.col("ss_sold_item_sk"),
                         ...
                         pl.col("ss_sold_date_sk"),
                         pl.col("ss_sold_promo_sk"),
                         pl.col("sales_price"), pl.col("profit")])
                .join(sr.select([pl.col("sr_item_sk"),
                                 pl.col("sr_ticket_number"),
                                 pl.col("return_amt"),
                                 pl.col("net_loss")]),
                ...
                .join(tablereader.read_table("date", ["d_month_seq",
                                                      "d_date_sk"])
                      .filter(pl.col("d_month_seq") == 1207),
                      left_on = "ss_sold_date_sk", right_on = "d_date_sk")
                .join(tablereader.read_table("item", ["i_current_price",
                                                      "i_item_sk"])
                      .filter(pl.col("i_current_price") > 50),
                      left_on = "ss_sold_item_sk", right_on = "i_item_sk")
                .join(tablereader.read_table("promotion", ["p_channel_tv",
                                                           "p_promo_sk"])
        ...
        csr2 = ExecutionNode("csr2",
            lambda cs, cr: cs
                .select([pl.col("cs_sold_item_sk"),
                         pl.col("cs_order_number"),
                         pl.col("cs_catalog_page_sk"),
                         pl.col("cs_sold_date_sk"),
                         pl.col("cs_promo_sk"),
                         pl.col("sales_price"), pl.col("profit")])
                .join(cr.select([pl.col("cr_item_sk"),
        ...
        wsr2 = ExecutionNode("wsr2",
            lambda ws: ws
                .select([pl.col("ws_item_sk"),
                         pl.col("ws_order_number"),
                         pl.col("wsr_web_site_sk"),
                         pl.col("date_sk"),
                         pl.col("ws_promo_sk"),
                         pl.col("sales_price"), pl.col("profit")])
    ...
    if job_num == 2:
        ...
        category_q56 = ExecutionNode("category_q56",
            lambda: tablereader.read_table("item")
                .filter(pl.col("i_color")
                        .is_in(["turquoise", "smoke", "slate", "thistle",
        ...
        ss = ExecutionNode("ss",
            lambda: tablereader.read_table("store_sales_2001",
                                           ["ss_sold_item_sk",
                                            "ss_sold_date_sk",
                                            "ss_sold_addr_sk",
                                            "ss_pricing_ext_sales_price",
                                            ...
                                            "ss_sold_customer_sk",
                                            "ss_sold_promo_sk"])
                .join(tablereader.read_table("item", ["i_item_sk",
                                                      "i_manufact_id",
                                                      "i_item_id"]),
                      left_on = "ss_sold_item_sk", right_on = "i_item_sk")
                .join(tablereader.read_table("date", ["d_year", "d_moy",
                                                      "d_date_sk"])
                      .filter(pl.col("d_moy") == 2),
        ...
        cs = ExecutionNode("cs",
            lambda: tablereader.read_table("catalog_sales_2001",
                                           ["cs_sold_item_sk",
                                            "cs_sold_date_sk",
                                            "cs_bill_addr_sk",
                                            "cs_pricing_ext_sales_price"])
        ...
        ws = ExecutionNode("ws",
            lambda: tablereader.read_table("web_sales_2001",
                                           ["ws_item_sk",
                                            "ws_sold_date_sk",
                                            "ws_bill_addr_sk",
                                            "ws_pricing_ext_sales_price"])
        ...
        ss_q61 = ExecutionNode("ss_q61",
            lambda ss, category_q61: ss
                ...
                .join(tablereader.read_table("store",
                      ...
                      right_on = "w_store_sk")
                .join(tablereader.read_table("customer", ["c_customer_sk"]),
    ...
    if job_num == 3:
        ...
        wswscs = ExecutionNode("wswscs",
            lambda stack: stack.filter(pl.col("store_sk") == -1)
                .select([pl.col("sold_date_sk"),
                         pl.col("sales_price")]).distinct()
        ...
        ss_base = ExecutionNode("ss_base",
            lambda: tablereader.read_table("store_sales_2000",
                ["ss_sold_item_sk", "ss_sold_date_sk", "ss_ticket_number",
                 "ss_pricing_quantity", "ss_pricing_net_paid",
                 "ss_pricing_ext_sales_price", "ss_sold_customer_sk",
                 "ss_pricing_sales_price", "ss_sold_store_sk"]).vstack(
                     tablereader.read_table("store_sales_2001",
                ["ss_sold_item_sk", "ss_sold_date_sk", "ss_ticket_number",
                 "ss_pricing_quantity", "ss_pricing_net_paid",
                 "ss_pricing_ext_sales_price", "ss_sold_customer_sk",
                 "ss_pricing_sales_price", "ss_sold_store_sk"])),
            [], tablereader)

        ws_base = ExecutionNode("ws_base",
            lambda: tablereader.read_table("web_sales_2000",
                ["ws_item_sk", "ws_sold_date_sk", "ws_order_number",
                 "ws_pricing_quantity", "ws_pricing_net_paid",
                 "ws_pricing_ext_sales_price", "ws_bill_customer_sk"]).vstack(
                     tablereader.read_table("web_sales_2001",
                ["ws_item_sk", "ws_sold_date_sk", "ws_order_number",
                 "ws_pricing_quantity", "ws_pricing_net_paid",
                 "ws_pricing_ext_sales_price", "ws_bill_customer_sk"])),
            [], tablereader)

        cs_base = ExecutionNode("cs_base",
            lambda: tablereader.read_table("catalog_sales_2000",
                ["cs_sold_item_sk", "cs_sold_date_sk", "cs_order_number",
                 "cs_pricing_quantity", "cs_pricing_ext_sales_price"]).vstack(
                     tablereader.read_table("catalog_sales_2001",
                ["cs_sold_item_sk", "cs_sold_date_sk", "cs_order_number",
                 "cs_pricing_quantity", "cs_pricing_ext_sales_price"])),
            [], tablereader)

        ws_q74 = ExecutionNode("ws_q74",
            lambda ws_base: ws_base
                .select([pl.col("ws_bill_customer_sk"),
        ...
        ss_q74 = ExecutionNode("ss_q74",
            lambda ss_base: ss_base
                .select([pl.col("ss_sold_customer_sk"),
                         pl.col("ss_sold_date_sk"),
    ...
    if job_num == 4:
        ss_base = ExecutionNode("ss_base",
            lambda: tablereader.read_table("store_sales",
                ["ss_sold_item_sk", "ss_sold_date_sk", "ss_pricing_net_profit",
                 "ss_pricing_quantity", "ss_pricing_net_paid",
                 "ss_ticket_number"]),
            [], tablereader)                                
        ...
        in_store = ExecutionNode("in_store",
            lambda ss_base: ss_base
                ...
                .select([pl.col("ss_ticket_number"),
                         pl.col("ss_sold_item_sk"),
                         pl.col("ss_sold_date_sk"),
                         pl.col("ss_pricing_quantity"),
                         pl.col("ss_pricing_net_paid")])
    ...
    if job_num == 5:
        ...
        # Query 14
        ss_base = ExecutionNode("ss_base",
            lambda: tablereader.read_table("store_sales_1999",
                                           ["ss_sold_date_sk",
                                            "ss_sold_item_sk",
                                            "ss_pricing_quantity",
                                            "ss_pricing_list_price",
                                            "ss_sold_customer_sk",
                                            "ss_pricing_sales_price"]).vstack(
                                                tablereader.read_table("store_sales_2000",
                                           ["ss_sold_date_sk",
                                            "ss_sold_item_sk",
                                            "ss_pricing_quantity",
                                            "ss_pricing_list_price",
        ...
        cs_base = ExecutionNode("cs_base",
            lambda: tablereader.read_table("catalog_sales_1999",
                                           ["cs_sold_date_sk",
                                            "cs_sold_item_sk",
                                            "cs_pricing_quantity",
                                ...
                                .vstack(tablereader.read_table("catalog_sales_2000",
                                           ["cs_sold_date_sk",
                                            "cs_sold_item_sk",
                                            "cs_pricing_quantity",
                                            "cs_pricing_list_price",
        ...
        ws_base = ExecutionNode("ws_base",
            lambda: tablereader.read_table("web_sales_1999",
                                           ["ws_sold_date_sk",
                                            "ws_item_sk",
                                            "ws_pricing_quantity",
                                            "ws_pricing_list_price",
                                            "ws_bill_customer_sk",
                                            "ws_pricing_sales_price"])
                                .vstack(tablereader.read_table("web_sales_2000",
                                           ["ws_sold_date_sk",
                                            "ws_item_sk",
                                            "ws_pricing_quantity",
                                            "ws_pricing_list_price",
        ...
        cross_items = ExecutionNode("cross_items",
            lambda intersect: intersect
                .join(tablereader.read_table("item", ["i_brand_id",
                                                      "i_class_id",
                                                      "i_category_id",
                                                      "i_item_sk"]),
        ...
        this_year = ExecutionNode("this_year",
            lambda ss_base, cross_items, avg_sales: ss_base
                .filter(pl.col("d_week_seq").is_in(tablereader
                                                    .read_table("date",
                                                                ["d_year",
                                                                 "d_week_seq"])
                                     .filter(pl.col("d_year") == 2000)
